...
# train model
tempData_NonWinter = None
for key, val in tempDataDist.items():
    val = val.loc[2:5]
    if '2023' in val.columns:
        val.drop('2023', axis=1, inplace=True)
    val = val.transpose()
    if tempData_NonWinter is None:
        tempData_NonWinter = val
    else:
        tempData_NonWinter = pd.concat([tempData_NonWinter, val])
tempData_NonWinter.reset_index(inplace=True, drop=True)
...
pred = 0
for i in range(100):
    pred += svm.predict([[13.32258, 16.28, 19.21935]])
